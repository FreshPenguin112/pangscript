...
class AstBuilder extends PangVisitor {
  ...
  inferType(node) {
    ...
    if (node.type === "Object") {
      ...
      for (const p of node.props) {
        ...
        let t = p.keyType ? this.normalizeTypeName(p.keyType) : null;
        if (!t) t = this.inferType(p && p.value ? p.value : null);
    ...
    if (node.type === "MemberCall") {
      const methodName = Array.isArray(node.chain) && node.chain.length >= 2
        ? node.chain[node.chain.length - 1] : null;
      if (methodName === "toString") return "String";
      if (["toUpperCase", "toLowerCase", "charAt", "concat", "substring", "slice", "trim", "repeat", "replace", "padStart", "padEnd"].includes(methodName)) return "String";
      if (methodName === "length" || methodName === "charCodeAt" || methodName === "indexOf" || methodName === "lastIndexOf") return "Number";
      if (["includes", "startsWith", "endsWith"].includes(methodName)) return "Boolean";
      return "Any";
    }
    if (node.type === "Call") return "Any";
    if (node.type === "Binary") {
      const op = node.op;
  ...
  visitArrowFunction(ctx) {
    // arrowFunction : '(' (paramDecl (',' paramDecl)*)? ')' typeAnnotation? '=>' (block | inlineBlock)
    ...
    let returnType = null;
    try {
      const ta = ctx.typeAnnotation ? ctx.typeAnnotation() : null;
      if (ta) {
        const taInfo = this.visit(ta);
        if (taInfo) returnType = taInfo;
      }
    } catch (e) { }
  ...
  visitPrimary(ctx) {
    ...
    // Postfix property access and method calls: .prop(args?) .prop(args?) ...
    if (ctx.children) {
      let current = base;
      for (let i = 0; i < ctx.children.length; i++) {
        const ch = ctx.children[i];
            ...
            if (txt === "(") {
              ...
              for (let ak = k + 1; ak < ctx.children.length; ak++) {
                ...
                if (
                  ...
                  argExprs.push(this.visit(inner));
                }
              }
              break;
            }
...
function performStaticTypeAnalysis(rootAst, lenientMode = false) {
  ...
  function inferPropertyTypeFromShape(type, propertyName) {
    ...
    if (normalized === "Number" && propertyName === "toString") return "String";
    if (normalized === "String") {
      if (["toString", "toUpperCase", "toLowerCase", "charAt", "concat", "substring", "slice", "trim", "repeat", "replace", "padStart", "padEnd"].includes(propertyName)) return "String";
    ...
    switch (node.type) {
      ...
      case "Object": {
        ...
        for (const p of node.props) {
          ...
          let t = p.keyType ? normalizeTypeName(p.keyType) : null;
          if (!t) t = visitExpression(p && p.value ? p.value : null);
      ...
      case "Lambda": {
        ...
        if (node.body && Array.isArray(node.body.body)) {
          ...
          visitStatements(node.body.body, lambdaScope, lambdaReturnType);
          scopes.pop();
        }
        return node.inferredType;
      }
    ...
    switch (stmt.type) {
      ...
      case "Method": {
        ...
        if (stmt.body && stmt.body.body) {
          ...
          visitStatements(stmt.body.body, methodScope, returnType);
          scopes.pop();
        }
        break;
      }
...
if (!nestedInput) {
  ...
  try {
    ...
    try {
      ...
      // Also load and normalize helpers so their lambda bodies get type inference
      try {
        var mathHelperAst = loadMathHelperAst();
        var stringHelperAst = loadStringHelperAst();
      } catch (eh) {}
      try {
        performStaticTypeAnalysis(ast);
        // Also run type analysis on helper bodies so their lambda param types
        // (e.g. `str` in toLowerCase) are inferred for string bracket access.
        if (stringHelperAst && Array.isArray(stringHelperAst)) {
          for (const node of stringHelperAst) {
            try {
              if (node && node.type === "On" && node.body && node.body.body)
                performStaticTypeAnalysis(node.body.body, true);
              else performStaticTypeAnalysis([node], true);
            } catch (_) {}
          }
        }
        if (mathHelperAst && Array.isArray(mathHelperAst)) {
          for (const node of mathHelperAst) {
            try {
              if (node && node.type === "On" && node.body && node.body.body)
                performStaticTypeAnalysis(node.body.body, true);
              else performStaticTypeAnalysis([node], true);
  ...
  // Pre-load helpers so their variables are registered in varToLambda and globalNames
  const preloadHelperAsts = [];
  try {
    const mathHelp = loadMathHelperAst();
    if (Array.isArray(mathHelp)) preloadHelperAsts.push(...mathHelp);
  } catch (e) {}
  try {
    const strHelp = loadStringHelperAst();
    if (Array.isArray(strHelp)) preloadHelperAsts.push(...strHelp);
  } catch (e) {}
  const combinedAst = preloadHelperAsts.length > 0 ? ast.concat(preloadHelperAsts) : ast;
  collectGlobalVariables(combinedAst);
  ...
  // Pre-register Math global name and load helper AST for lambda registration
  const MATH_METHOD_NAMES = ["abs", "floor", "ceil", "round", "sqrt", "sign", "trunc", "sin", "cos", "tan", "asin", "acos", "atan", "atan2", "exp", "log", "log10", "log2", "pow", "min", "max", "random", "cbrt", "hypot"];
  globalNames.add("Math");
  globalNames.add("_INTERNAL_numberToString");
  const MATH_PARAMS_1 = ["n"];
  const MATH_PARAMS_2 = ["x", "y"];
  const MATH_PARAMS_0 = [];
  const MATH_PARAMS_ATAN2 = ["y", "x"];
  for (const method of MATH_METHOD_NAMES) {
    let params;
    if (method === "atan2") params = MATH_PARAMS_ATAN2;
    else if (method === "pow" || method === "min" || method === "max" || method === "hypot") params = MATH_PARAMS_2;
    else if (method === "random") params = MATH_PARAMS_0;
    else params = MATH_PARAMS_1;
    const key = `Math.${method}`;
    const arr = params.map((_, i) => `__arg${i}`);
    const map = {};
    params.forEach((p, i) => { map[p] = `__arg${i}`; });
    varToLambda[key] = { arr, map, params: params.slice() };
  }
  varToLambda["_INTERNAL_numberToString"] = {
    arr: ["__arg0", "__arg1"],
    map: { n: "__arg0", radix: "__arg1" },
    params: ["n", "radix"],
  };
  ...
  // If an arg expression reads a temp that will be overwritten by a later setter
  // (e.g. `_stringIndexOf` calls `_stringMatchAt(str, i, search)` where `search`
  //  maps to `__arg1` which is overwritten by the setter for `i` → `__arg1`),
  // the value is saved to a unique temp first to prevent data corruption.
  function buildLambdaArgSetters(vinfo, args, inMethod, paramMap) {
    ...
    for (let i = 0; i < count; i++) {
      targets[i] = hasNamed ? vinfo.arr[i] : `__arg${i}`;
      argExprs[i] = args && args[i] ? exprToNested(args[i], inMethod, paramMap) : "";
    }
    for (let i = 0; i < count; i++) {
      const argExpr = argExprs[i];
      if (argExpr && argExpr.type === "Var" && targets.slice(0, i).includes(argExpr.name)) {
        const safeName = `__argsa_${i}_${targets[i]}`;
        setters.push({ opcode: "SPtempVars_setVar", inputs: ["thread", safeName, argExpr] });
        argExprs[i] = { type: "Var", name: safeName, scope: "thread" };
      }
    }
    for (let i = 0; i < count; i++) {
      setters.push({ opcode: "SPtempVars_setVar", inputs: ["thread", targets[i], argExprs[i]] });
    }
    return setters;
  }
  ...
  collectVarAssignments(combinedAst);
  ...
  let runtimeTypeofHelperAst = null;
  function loadRuntimeTypeofHelperAst() {
    if (runtimeTypeofHelperAst) return runtimeTypeofHelperAst;
    const helperPath = path.join(__dirname, "helpers", "runtime_typeof.pang");
    const source = fs.readFileSync(helperPath, "utf8");
    const ast = parseWithAntlr(source);
    normalizeAstArrays(ast);
    runtimeTypeofHelperAst = ast;
    return ast;
  }
  ...
  var mathHelperAst = null;
  function loadMathHelperAst() {
    if (mathHelperAst) return mathHelperAst;
    const helperPath = path.join(__dirname, "helpers", "Math2.pang");
    const source = fs.readFileSync(helperPath, "utf8");
    const ast = parseWithAntlr(source);
    normalizeAstArrays(ast);
    mathHelperAst = ast;
  ...
  var stringHelperAst = null;
  function loadStringHelperAst() {
    if (stringHelperAst) return stringHelperAst;
    const helperPath = path.join(__dirname, "helpers", "String.pang");
    const source = fs.readFileSync(helperPath, "utf8");
    const ast = parseWithAntlr(source);
    normalizeAstArrays(ast);
    stringHelperAst = ast;
  ...
  function exprToNested(expr, inMethod = false, paramMap = null) {
    ...
    if (expr.type === "MemberCall") {
      ...
      if (typeof first === "string") {
        objReceiver =
          first === "this"
            ? selfRef()
            : mapReceiverName(first, paramMap);
      } else if (first && first.type === "Var") {
        objReceiver = mapReceiverName(first.name, paramMap);
      } else {
      ...
      // Primitive toString method: handle number.toString() and number.toString(radix)
      if (methodName === "toString") {
        // When chain[0] is an expression (not a string), evaluate it first
        let receiverNested = objReceiver;
        if (!receiverNested && chain.length >= 2 && typeof chain[0] === "object") {
          receiverNested = exprToNested(chain[0], inMethod, paramMap);
        }
        if (!expr.args || expr.args.length === 0) {
          return { opcode: "operator_stringify", inputs: [receiverNested] };
        }
        const radixArg = exprToNested(expr.args[0], inMethod, paramMap);
        const nArg = receiverNested;
        const lambdaGetter = { opcode: "SPtempVars_getVar", inputs: ["global", "_INTERNAL_numberToString"] };
        const setters = [
          { opcode: "SPtempVars_setVar", inputs: ["thread", "__arg0", nArg] },
          { opcode: "SPtempVars_setVar", inputs: ["thread", "__arg1", radixArg] },
        ];
        const callMethod = { opcode: "jwLambda_executeR", inputs: [lambdaGetter, ""] };
        const ret = { opcode: "procedures_return", inputs: [callMethod] };
        return { opcode: "control_inline_stack_output", inputs: [setters.concat([ret])] };
      }

      // String instance methods: route to _INTERNAL_String.method(receiver, args...)
      const stringMethods = new Set([
        "charAt", "charCodeAt", "length", "concat", "substring", "slice",
        "indexOf", "lastIndexOf", "includes", "startsWith", "endsWith",
        "trim", "toUpperCase", "toLowerCase", "repeat", "replace", "padStart", "padEnd",
      ]);
      if (stringMethods.has(methodName)) {
        ...
        // Build method getter: get _INTERNAL_String, then get the method property
        const strObjGetter = { opcode: "SPtempVars_getVar", inputs: ["global", "_INTERNAL_String"] };
        const methodGetter = { opcode: "dogeiscutObject_get", inputs: [{ ...strObjGetter, noPlaceholder: true }, methodName] };
        // Set __arg0 = receiver, __arg1...N = remaining args
        const setters = [
          { opcode: "SPtempVars_setVar", inputs: ["thread", "__arg0", receiverNested] },
        ];
        if (expr.args && expr.args.length > 0) {
          for (let ai = 0; ai < expr.args.length; ai++) {
            const argNested = exprToNested(expr.args[ai], inMethod, paramMap);
            setters.push({ opcode: "SPtempVars_setVar", inputs: ["thread", `__arg${ai + 1}`, argNested] });
          }
        }
        ...
        function helper(n) {
          ...
          if (typeof first === "object") {
            if (first.type === "Var") {
              ...
              steps.push({ receiverChain: [first.name], methodName: method, args: n.args || [] });
              return;
            }
  ...
  function stmtToNested(stmt, inMethod = false, paramMap = null) {
    ...
    if (stmt.type === "MemberCall") {
      ...
      // Primitive toString method (statement context)
      if (methodName === "toString") {
        if (!stmt.args || stmt.args.length === 0) {
          const first = chain[0];
          const recv = typeof first === "string"
            ? (first === "this" ? selfRef() : mapReceiverName(first, paramMap))
            : "";
          return { opcode: "operator_stringify", inputs: [recv] };
        }
        return null;
      }

      // String instance methods (statement context)
      const stringMethods = new Set([
        "charAt", "charCodeAt", "length", "concat", "substring", "slice",
        "indexOf", "lastIndexOf", "includes", "startsWith", "endsWith",
        "trim", "toUpperCase", "toLowerCase", "repeat", "replace", "padStart", "padEnd",
      ...
      if (stringMethods.has(methodName)) {
        stringUsed = true;
        let receiverNested = null;
        if (chain.length >= 2 && typeof chain[0] === "string") {
          receiverNested = chain[0] === "this" ? selfRef() : mapReceiverName(chain[0], paramMap);
        } else if (typeof chain[0] === "object") {
          receiverNested = exprToNested(chain[0], inMethod, paramMap);
        }
        if (!receiverNested) return null;
        const strObjGetter = { opcode: "SPtempVars_getVar", inputs: ["global", "_INTERNAL_String"] };
        const methodGetter = { opcode: "dogeiscutObject_get", inputs: [{ ...strObjGetter, noPlaceholder: true }, methodName] };
        const setters = [
          { opcode: "SPtempVars_setVar", inputs: ["thread", "__arg0", receiverNested] },
        ...
        if (stmt.args && stmt.args.length > 0) {
          for (let ai = 0; ai < stmt.args.length; ai++) {
            const argNested = exprToNested(stmt.args[ai], inMethod, paramMap);
            setters.push({ opcode: "SPtempVars_setVar", inputs: ["thread", `__arg${ai + 1}`, argNested] });
          }
        }
        const callMethod = { opcode: "jwLambda_executeR", inputs: [methodGetter, ""] };
  ...
  if (runtimeTypeofHelperUsed && !userDefinedRuntimeTypeof) {
    const helperAst = loadRuntimeTypeofHelperAst();
    const helperOn = helperAst.find((n) => n && n.type === "On" && n.event === "flag");
    if (!helperOn) throw new Error("runtime_typeof.pang: expected an on('flag') handler");
    arraysUsed = true;
    objectsUsed = true;
    lambdasUsed = true;
    usedClasses = true;
  ...
  // Always inject the Math helper
  {
    const mathAst = loadMathHelperAst();
    const mathOn = mathAst.find((n) => n && n.type === "On" && n.event === "flag");
    if (!mathOn) throw new Error("Math.pang: expected an on('flag') handler");
    arraysUsed = true;
    objectsUsed = true;
    lambdasUsed = true;
    usedClasses = true;
    ...
    const rawChildren =
      mathOn.body && mathOn.body.body
        ? mathOn.body.body.map((c) => stmtToNested(c, false)).filter(Boolean)
        : [];
    const children = [];
    for (const c of rawChildren) {
      if (Array.isArray(c)) children.push(...c);
  ...
  // Always inject the String helper
  {
    const strAst = loadStringHelperAst();
    const strOn = strAst.find((n) => n && n.type === "On" && n.event === "flag");
    if (!strOn) throw new Error("String.pang: expected an on('flag') handler");
    arraysUsed = true;
    objectsUsed = true;
    lambdasUsed = true;
    const rawChildren =
      strOn.body && strOn.body.body
        ? strOn.body.body.map((c) => stmtToNested(c, false)).filter(Boolean)
        : [];
    const children = [];
    for (const c of rawChildren) {
      if (Array.isArray(c)) children.push(...c);
...
if (emitResult && emitResult.pmOperatorsExpansion_used || pmOperatorsExpansion_used) {
  out.extensions = out.extensions || [];
