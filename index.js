...
class AstBuilder extends PangVisitor {
  ...
  visitExpr(ctx) {
    ...
    // Ternary conditional: expr '?' expr ':' expr
      ...
      const hasQuestion = ctx.children.some(c => c && typeof c.getText === 'function' && c.getText() === '?');
      if (hasQuestion) {
      ...
      if (valCtx.STRING && valCtx.STRING()) {
        ...
        try {
          ...
          if (valCtx.expr && valCtx.expr()) {
            ...
            const exprCtx = typeof valCtx.expr === 'function' ? (Array.isArray(valCtx.expr()) ? valCtx.expr(0) : valCtx.expr()) : null;
            if (exprCtx) {
...
function emitProject(astStmts) {
  ...
  function emitSetVar(name, valueExpr, allowConstInit = false) {
    ...
    const info = varInfo[name] || ensureVariable(name, 'let', valueExpr && valueExpr.type === 'Literal' ? valueExpr : null);
    ...
    blocks[id] = { opcode: 'SPtempVars_setVar', next: null, parent: null, inputs, fields, shadow: false, topLevel: false };
  ...
  function emitGetVar(name) {
    ...
    const fields = { TYPE: ['global', info.typeId] };
    blocks[id] = { opcode: 'SPtempVars_getVar', next: null, parent: null, inputs, fields, shadow: false, topLevel: false };
  ...
  function emitCallStmt(stmt) {
      ...
      if (stmt.args && stmt.args.length > 0) {
        for (let i = 0; i < stmt.args.length; i++) {
          ...
          if (!a) {
            ...
            if (param.type === 'number' || a.litType === 'number') inputs[inName] = [1, [InputTypes.math_number, String(a.value)]];
            ...
            else { inputs[inName] = [3, emitted.id, makeShadowForExpr(a)]; blocks[emitted.id].parent = id; }
          }
        }
      }
    } else {
    ...
    blocks[id] = { opcode: stmt.name, next: null, parent: null, inputs, fields: {}, shadow: false, topLevel: false };
    return { first: id, last: id };
  }
  ...
  function emitDeclare(stmt) {
    ...
    if (stmt.value) {
      // emit initial set
      const val = (stmt.value.type === 'Literal') ? { type: 'lit', value: (stmt.value.litType === 'number' ? [1, [InputTypes.math_number, String(stmt.value.value)]] : [1, [InputTypes.text, String(stmt.value.value)]]) } : emitExpression(stmt.value);
  ...
  function emitAssign(stmt) {
    ...
    const value = stmt.value.type === 'Literal' ? { type: 'lit', value: (stmt.value.litType === 'number' ? [1, [InputTypes.math_number, String(stmt.value.value)]] : [1, [InputTypes.text, String(stmt.value.value)]]) } : emitExpression(stmt.value);
  ...
  function mapOpToOpcode(op) {
    switch (op) {
      ...
      case '>=': return 'operator_gtorequal';
      case '<=': return 'operator_ltorequal';
      case '==':
      case '===': return 'operator_equals';
      case '!=':
      case '!==': return 'operator_notequal';
      ...
      case '**': return 'operator_power';
      default: throw new Error('emit: unsupported operator ' + op);
    }
  }
  ...
  function emitExpression(exprNode) {
    ...
    if (exprNode.type === 'Ternary') {
      ...
      if (condRes.type !== 'id') {
        ...
        const boolOpcode = (condRes.type === 'lit' && condRes.value && condRes.value[1] && String(condRes.value[1][1]) === 'true') ? 'operator_trueBoolean' : 'operator_falseBoolean';
        blocks[boolId] = { opcode: boolOpcode, next: null, parent: null, inputs: {}, fields: {}, shadow: true, topLevel: true };
      ...
      blocks[checkboxId] = { opcode: 'checkbox', next: null, parent: null, inputs: {}, fields: { CHECKBOX: ['FALSE', checkboxUnique] }, shadow: true, topLevel: true, x: 329, y: 425 };
      ...
      if (!exprNode.thenExpr) inputs.TEXT1 = [1, [InputTypes.text, '']];
        ...
        inputs.TEXT1 = (t.litType === 'number') ? [1, [InputTypes.math_number, String(t.value)]] : [1, [InputTypes.text, String(t.value)]];
      } else {
        ...
        else { inputs.TEXT1 = [3, thenRes.id, makeShadowForExpr(exprNode.thenExpr)]; blocks[thenRes.id].parent = id; }
      }
      ...
      if (!exprNode.elseExpr) inputs.TEXT2 = [1, [InputTypes.text, '']];
        ...
        inputs.TEXT2 = (t.litType === 'number') ? [1, [InputTypes.math_number, String(t.value)]] : [1, [InputTypes.text, String(t.value)]];
      } else {
        ...
        else { inputs.TEXT2 = [3, elseRes.id, makeShadowForExpr(exprNode.elseExpr)]; blocks[elseRes.id].parent = id; }
      }

      blocks[id] = { opcode: 'control_if_return_else_return', next: null, parent: null, inputs, fields: {}, shadow: false, topLevel: false };
    ...
    if (exprNode.type === 'Literal') {
      if (exprNode.litType === 'boolean') {
        ...
        const id = genId();
        blocks[id] = { opcode: boolOpcode, next: null, parent: null, inputs: {}, fields: {}, shadow: true, topLevel: true };
        return { type: 'id', id, node: exprNode };
      }
      if (exprNode.litType === 'number') return { type: 'lit', value: [1, [InputTypes.math_number, String(exprNode.value)]] };
    ...
    if (exprNode.type === 'Call') {
      ...
      if (exprNode.name === 'ask') {
        ...
        blocks[answerId] = { opcode: 'sensing_answer', next: null, parent: null, inputs: {}, fields: {}, shadow: false, topLevel: false };

        // create procedures_return block (returns the answer reporter)
        const retId = genId();
        blocks[retId] = { opcode: 'procedures_return', next: null, parent: null, inputs: { return: [3, answerId, [InputTypes.text, '1']] }, fields: {}, shadow: false, topLevel: false };
        ...
        if (arg && arg.type === 'Literal') {
          qInput = arg.litType === 'number' ? [1, [InputTypes.math_number, String(arg.value)]] : [1, [InputTypes.text, String(arg.value)]];
        } else if (arg) {
          const emitted = emitExpression(arg);
          if (emitted && emitted.type === 'lit') qInput = emitted.value;
          else if (emitted && emitted.type === 'id') { qInput = [3, emitted.id, makeShadowForExpr(arg)]; blocks[emitted.id].parent = askId; }
        }
        blocks[askId] = { opcode: 'sensing_askandwait', next: retId, parent: null, inputs: { QUESTION: qInput }, fields: {}, shadow: false, topLevel: false };
        blocks[retId].parent = askId;

        // create inline stack reporter block that wraps SUBSTACK pointing to askId
        const inlineId = genId();
        blocks[inlineId] = { opcode: 'control_inline_stack_output', next: null, parent: null, inputs: { SUBSTACK: [2, askId] }, fields: {}, shadow: false, topLevel: false };
        ...
        if (exprNode.args && exprNode.args.length > 0) {
          for (let i = 0; i < exprNode.args.length; i++) {
            ...
            if (!a) {
              ...
              if (emitted.type === 'lit') inputs[inName] = emitted.value; else { inputs[inName] = [3, emitted.id, makeShadowForExpr(a)]; blocks[emitted.id].parent = id; }
            }
          }
        }
      } else {
      ...
      blocks[id] = { opcode: exprNode.name, next: null, parent: null, inputs, fields: {}, shadow: false, topLevel: false };
      return { type: 'id', id, node: exprNode };
    }
    if (exprNode.type === 'Unary') {
      ...
      if (operand.type === 'lit') inputs.OPERAND = operand.value; else { inputs.OPERAND = [3, operand.id, null]; blocks[operand.id].parent = id; }
      blocks[id] = { opcode, next: null, parent: null, inputs, fields: {}, shadow: false, topLevel: false };
      return { type: 'id', id, node: exprNode };
    }
    if (exprNode.type === 'Binary') {
      ...
      if (exprNode.op === '..') {
        ...
        if (!allNumericLiterals) {
          ...
          blocks[joinId] = { opcode: 'operator_expandablejoininputs', next: null, parent: null, inputs, fields: {}, shadow: false, topLevel: false, mutation };
  ...
  const opForBinary = (exprNode.op === '..' && typeof allNumericLiterals !== 'undefined' && allNumericLiterals) ? '+' : exprNode.op;
  const opcode = mapOpToOpcode(opForBinary);
      ...
      const mathOps = new Set(['operator_add','operator_subtract','operator_multiply','operator_divide','operator_power']);
      const leftName = mathOps.has(opcode) ? 'NUM1' : 'OPERAND1';
      const rightName = mathOps.has(opcode) ? 'NUM2' : 'OPERAND2';

      if (left.type === 'lit') inputs[leftName] = left.value; else { inputs[leftName] = [3, left.id, null]; if (!blocks[left.id]) throw new Error('emitExpression: missing left child ' + left.id); blocks[left.id].parent = id; }
      if (right.type === 'lit') inputs[rightName] = right.value; else { inputs[rightName] = [3, right.id, null]; if (!blocks[right.id]) throw new Error('emitExpression: missing right child ' + right.id); blocks[right.id].parent = id; }
  ...
  function emitIf(node) {
    ...
    for (let i = 0; i < node.cases.length; i++) {
      ...
      if (cres.type !== 'id') {
        const boolId = genId();
        const boolOpcode = (cres.type === 'lit' && cres.value && cres.value[1] && String(cres.value[1][1]) === 'true') ? 'operator_trueBoolean' : 'operator_falseBoolean';
        blocks[boolId] = { opcode: boolOpcode, next: null, parent: null, inputs: {}, fields: {}, shadow: true, topLevel: true };
        cres.id = boolId; cres.type = 'id';
      }
    ...
    for (let i = 0; i < condResArr.length; i++) {
      ...
      blocks[cid] = { opcode: 'checkbox', next: null, parent: null, inputs: {}, fields: { CHECKBOX: ['FALSE', unique] }, shadow: true, topLevel: true };
    ...
    blocks[ifId] = { opcode: 'control_expandableIf', next: null, parent: null, inputs, fields: {}, shadow: false, topLevel: false, mutation };
...
out.targets[1].variables = emitResult.variables || {};
out.targets[0].blocks = out.targets[0].blocks || {};

// Ensure SPtempVars extension declared if variables were created
if (Object.keys(out.targets[1].variables).length > 0) {
  ...
  out.extensionURLs.SPtempVars = out.extensionURLs.SPtempVars || 'https://sharkpools-extensions.vercel.app/extension-code/Temporary-Variables.js';
}
...
console.log(`✅ ${outJSONLocation.split(path.sep).pop()} written with`, Object.keys(emitResult.blocks).length, "blocks.");
