(function(Scratch) {
  ...
  const refreshDeps = () => {
    ...
    const runGenerator = gen => {
      ...
      class ClassType {
        constructor(construct = function*() {}, name = '', extension = null, proc = null, staticConstruct = null) {
          ...
          this.prototype = Object.create(extension?.prototype ?? null);
        }
    ...
    const jwClass = {
      ...
      setProp(name, pointer, value) {
        ...
        pointer.value = dogeiscutObject.Type.toObject(pointer.value);
      ...
      getProp(name, pointer) {
        ...
        const cls = pointer.value.map.get(classSymbol);
        if (!(cls instanceof ClassType)) return null;

        // Prototype first.
        if (name in cls.prototype) return cls.prototype[name];

        // Then instance fields.
        if (pointer.value.map.has(name)) return pointer.value.map.get(name);

        return null;
      },

      // These are now aliases onto the same prototype storage.
      setStatic(name, classType, value) {
        refreshDeps();
        classType = ClassType.toClass(classType);
        if (!(classType instanceof ClassType)) return;
        classType.prototype[name] = value;
      },
      ...
      getStatic(name, classType) {
        ...
        return classType.prototype[name];
      },

      setProto(name, classType, value) {
        refreshDeps();
        classType = ClassType.toClass(classType);
        if (!(classType instanceof ClassType)) return;
        classType.prototype[name] = value;
    ...
    class Extension {
      ...
      getInfo() {
        ...
        return {
          ...
          blocks: [
            ...
            {
              opcode: 'setProto',
              text: 'set prototype [NAME] on [CLASS] to [VALUE]',
              blockType: BlockType.COMMAND,
              arguments: {
                NAME: { type: ArgumentType.STRING, defaultValue: 'foo' },
                CLASS: jwClass.Argument,
            ...
            {
              opcode: 'getProto',
              text: 'get prototype [NAME] on [CLASS]',
              blockType: BlockType.REPORTER,
              arguments: {
                NAME: { type: ArgumentType.STRING, defaultValue: 'foo' },
                CLASS: jwClass.Argument
        ...
        return {
          ...
          js: {
            class: (node, compiler, imports) => {
              ...
              compiler.source = '(function() {\n';
                compiler.source += 'const _cls = new vm.jwClass.Type(function*(_jwClassSelf, thread, target) {\n';
                  ...
                  let nameExpr = compiler.descendInput(node.name).asUnknown().trim();
                  if (nameExpr.endsWith(';')) {
                    nameExpr = nameExpr.slice(0, -1);
                  }

                  compiler.source += `}, ${nameExpr}, null, thread.procedures, function*(_jwClassSelf, thread, target) {\n`;
            ...
            extend: (node, compiler, imports) => {
              let clsExpr = compiler.descendInput(node.class).asUnknown().trim();

              if (clsExpr.endsWith(";")) {
                clsExpr = clsExpr.slice(0, -1);
              }
              return new imports.TypedInput(
                `vm.jwClass.Type.toClass(${clsExpr}).extend(${compiler.descendInput(node.extension).asUnknown()})`,
                                            imports.TYPE_UNKNOWN
              );
            },
            ...
            setStatic: (node, compiler, imports) => {
              ...
              compiler.source += `vm.jwClass.setStatic(${compiler.descendInput(node.name).asUnknown()}, ${clsExpr}, ${compiler.descendInput(node.value).asUnknown()});\n`;
            },

            setProto: (node, compiler, imports) => {
              let clsExpr = compiler.descendInput(node.class).asUnknown().trim();

              if (clsExpr.endsWith(";")) {
                clsExpr = clsExpr.slice(0, -1);
              }
              compiler.source += `vm.jwClass.setProto(${compiler.descendInput(node.name).asUnknown()}, ${clsExpr}, ${compiler.descendInput(node.value).asUnknown()});\n`;
            },
            ...
            getStatic: (node, compiler, imports) => {
              ...
              return new imports.TypedInput(
                `vm.jwClass.getStatic(${compiler.descendInput(node.name).asUnknown()}, ${clsExpr})`,
                                            imports.TYPE_UNKNOWN
              );
            },

            getProto: (node, compiler, imports) => {
              let clsExpr = compiler.descendInput(node.class).asUnknown().trim();
              ...
              return new imports.TypedInput(
                `vm.jwClass.getProto(${compiler.descendInput(node.name).asUnknown()}, ${clsExpr})`,
                                            imports.TYPE_UNKNOWN
              );
            },
            ...
            new: (node, compiler, imports) => {
              ...
              return new imports.TypedInput(
                `(yield* vm.jwClass.Type.toClass(${clsExpr}).createInstance(thread, target))`,
                                            imports.TYPE_UNKNOWN
              );
            },
            ...
            getName: (node, compiler, imports) => {
              ...
              return new imports.TypedInput(
                `vm.jwClass.Type.toClass(${clsExpr}).name`,
                                            imports.TYPE_STRING
              );
            },
            ...
            instanceof: (node, compiler, imports) => {
              ...
              return new imports.TypedInput(
                `vm.jwClass.instanceOf(${compiler.descendInput(node.pointer).asUnknown()}, vm.jwClass.Type.toClass(${clsExpr}))`,
      ...
      class({ NAME }, util) {
        ...
        const cls = new jwClass.Type(
          instanceConstruct,
          NAME,
          null,
          util.thread?.procedures,
          staticConstruct
        );
      ...
      setProto({ NAME, CLASS, VALUE }) {
        refreshDeps();
        jwClass.setProto(NAME, CLASS, VALUE);
      }
      ...
      getProto({ NAME, CLASS }) {
        refreshDeps();
        return jwClass.getProto(NAME, CLASS);
      }
