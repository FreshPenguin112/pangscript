...
class generator {
    ...
    let project = this.template;
        ...
        // Safety: ensure setter-like jgJSON_* blocks are not emitted as standalone
        // stacked blocks. Some jgJSON opcodes perform mutation (set/insert/delete)
        // and must be wrapped by a `data_setvariableto` command. If any such block
        // does not already have a `data_setvariableto` ancestor, attempt to wrap
        // it using the block's inputs to discover the base variable name.
        const setterLike = new Set([
        ...
        for (const id of Object.keys(sanitized)) {
            const blk = sanitized[id];
            if (!blk || typeof blk.opcode !== 'string' || !blk.opcode.startsWith('jgJSON_')) continue;
            // Only consider setter-like opcodes here
            if (!setterLike.has(blk.opcode)) continue;
            // Walk ancestor chain to see if already under a data_setvariableto
            let anc = blk.parent;
            let hasSetterAncestor = false;
            while (anc) {
                const aBlk = sanitized[anc];
                if (!aBlk) break;
                if (aBlk.opcode === 'data_setvariableto') { hasSetterAncestor = true; break; }
                anc = aBlk.parent;
            }
            if (hasSetterAncestor) continue;
            // Attempt to derive base variable from blk inputs
            try {
                const inputs = blk.inputs || {};
                let candidate = null;
                if (inputs.array && Array.isArray(inputs.array) && inputs.array[0] === 3) candidate = inputs.array[1];
                else if (inputs.JSON && Array.isArray(inputs.JSON) && inputs.JSON[0] === 3) candidate = inputs.JSON[1];
                else if (inputs.JSON && inputs.JSON[0] === 3) candidate = inputs.JSON[1];
                if (candidate && sanitized[candidate] && sanitized[candidate].fields && Array.isArray(sanitized[candidate].fields.VARIABLE)) {
                    const baseVar = sanitized[candidate].fields.VARIABLE[0];
                    if (baseVar) {
                        const setId = this.letterCount(this.blockIdCounter++);
                        // Insert a setter block in the same chain position as the original jgJSON block.
                        // Preserve the original parent's `next` linkage where possible so the
                        // execution order remains unchanged.
                        const origParent = blk.parent || null;
                        const origNext = blk.next || null;
                        sanitized[setId] = {
                            opcode: 'data_setvariableto',
                            next: origNext,
                            parent: origParent,
                            inputs: { VALUE: [3, id, [10, '']] },
                            fields: { VARIABLE: [baseVar, baseVar, ''] },
                            shadow: false,
                            topLevel: false,
                            mutation: null
                        };
                        // If the original parent pointed to this block as its `next`, update it to point to the new setter
                        try {
                            if (origParent && sanitized[origParent] && sanitized[origParent].next === id) {
                                sanitized[origParent].next = setId;
                            }
                        } catch (e) { }
                        // Reparent the jgJSON block under the new setter and clear its next (reporters have no next)
                        blk.parent = setId;
                        blk.next = null;
                    }
                }
            } catch (e) {
                // ignore any failure here; don't block project generation
            }
        }
