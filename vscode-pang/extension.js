    ...
    if (!PangLexer || !PangParser || !antlr4) {
        ...
        if (serverModule && typeof serverModule.validateText === "function") {
            try {
                ...
                const sd = Array.isArray(serverResult)
                    ? serverResult
                    : serverResult.diagnostics || [];
                const declared =
                    serverResult && serverResult._declared
                        ? serverResult._declared
                        : sd._declared || new Map();
        ...
        diagnostics.push({
            ...
            message:
                "Pang parser not found. Generate parser with ANTLR and ensure lib/PangParser.js exists next to the extension or in the workspace lib/.",
            source: "pang",
        });
        return { diagnostics, _declared: new Map() };
    }
...
function registerProviders(context) {
    ...
    context.subscriptions.push(
        window.onDidChangeTextEditorSelection((e) => {
            if (
                e.textEditor.document.languageId === "pang" ||
                e.textEditor.document.fileName.endsWith(".ps")
            ) {
    ...
    context.subscriptions.push(
        workspace.onDidChangeTextDocument((e) => {
            if (
                e.document.languageId === "pang" ||
                e.document.fileName.endsWith(".ps")
            ) {
            ...
            { scheme: "file", language: "pang" },
            {
                provideCompletionItems(document, position) {
                    try {
                        ...
                        try {
                            ...
                            if (fs.existsSync(snippetPath)) {
                                ...
                                for (const key of Object.keys(sn)) {
                                    ...
                                    const insert = Array.isArray(s.body)
                                        ? s.body.join("\n")
                                        : s.body;
                                    items.push(
                        ...
                        const lines = text.split(/\r?\n/);
                        const declRe =
                            /\b(?:let|const)\s+([a-zA-Z_][a-zA-Z0-9_]*)/g;
                        const assignRe = /^\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*=/;
                        for (let i = 0; i < lines.length; i++) {
                            let m;
                    ...
                    provideHover(document, position) {
                        ...
                        while ((match = re.exec(line))) {
                            ...
                            if (
                                ...
                                // Regex to detect both multi-line ```pang blocks``` and single-line `code`
                                const codeRegex =
                                    /```pang\s*([\s\S]*?)```|`([^`\n]+)`/g;
                                let lastIndex = 0;
                                let cbMatch;
                                ...
                                while (
                                    (cbMatch = codeRegex.exec(docText)) !== null
                                ) {
                                    ...
                                    if (cbMatch[1] !== undefined) {
                                        // Multi-line ```pang```
                                        md.appendCodeblock(cbMatch[1], "pang");
                                    } else if (cbMatch[2] !== undefined) {
                                        // Single-line `code` — use appendCodeblock to force syntax highlighting
                                        md.appendCodeblock(cbMatch[2], "pang");
                                    }

                                    lastIndex =
                                        cbMatch.index + cbMatch[0].length;
                                }
