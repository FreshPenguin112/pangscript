...
function makeShadowForParam(param) {
  // For now, create simple literal shadows for number/string reporters
  if (!param) return null;
  if (param.type === "substack") return null;
  // use reported shape: treat as string/number reporter
  return {
    opcode: param.field || "",
    inputs: [],
    fields: {}
    ...
    for (let i = 0; i < params.length; i++) {
      ...
      if (p.type === "substack") {
        ...
        if (val && typeof val === "object" && val.opcode) {
          ...
          // literal -> encode as a shadow value [typeCode, stringValue]
          const typeCode = typeCodeForParam(p);
          if (!p.field) {
