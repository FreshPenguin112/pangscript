...
function isCurrentObjectBlock(val) {
  if (!val || typeof val !== "object") return false;
  const opcode = String(val.opcode || "").trim().toLowerCase();
  return opcode === "dogeiscutobject_currentobject" || opcode === "dogeiscutobject_current_object" || opcode === "currentobject" || opcode === "current_object";
}
  ...
  function createBlock(obj, parent = null) {
    ...
    for (let i = 0; i < params.length; i++) {
      ...
      if (p.type === "substack") {
        ...
        // The first child is nested under the parent block, and each subsequent child
        // is parented to the previous child in the substack chain.
        const created = [];
        ...
        if (Array.isArray(val)) {
          for (let j = 0; j < val.length; j++) {
            ...
            const childEntry = createBlock(ch, id);
            blocksOut[childEntry.id].parent = previousChildId || id;
            blocksOut[childEntry.id].topLevel = false;
            if (previousChildId) {
              blocksOut[previousChildId].next = childEntry.id;
            }
            created.push(childEntry.id);
            previousChildId = childEntry.id;
          }
        }
        ...
        if (val && typeof val === "object" && val.opcode) {
          ...
          // Special-case: builder CURRENT_OBJECT should be serialized as a direct block reference input.
          if (!p.field && String(p.name).toUpperCase() === 'CURRENT_OBJECT' && isCurrentObjectBlock(val)) {
            entry.inputs[p.name] = [1, child.id];
          } else if (!p.field) {
          ...
          const typeCode = obj.opcode === 'dogeiscutObject_builderAppend' || obj.opcode === 'dogeiscutObject_builderAppendEmpty' ? InputTypes.text : typeCodeForParam(p);
          ...
          if (!p.field) {
            ...
            if (obj.opcode === 'jwClass_getProp' && String(p.name).toUpperCase() === 'POINTER' && isEmpty) {
              ...
              if (shadowSpec) {
                ...
                if (typeof val === "number") entry.inputs[p.name] = [1, [typeCode, String(val)]];else if (typeof val === "string") entry.inputs[p.name] = [1, [typeCode, val]];else entry.inputs[p.name] = [1, [typeCode, String(val)]];
              }
            }
          }
