  ...
  function createBlock(obj, parent = null) {
    ...
    if (obj.opcode === "control_expandableIf" && Array.isArray(obj.cases)) {
      ...
      for (let i = 0; i < cases.length; i++) {
        ...
        if (c.cond && typeof c.cond === "object" && c.cond.opcode) {
          ...
          // if the supplied child requests no placeholder, omit the default checkbox
          if (c.cond.noPlaceholder) entry.inputs["BOOL" + idx] = [3, condChild.id];else entry.inputs["BOOL" + idx] = [3, condChild.id, checkboxIds[i]];
        } else {
        ...
        if (Array.isArray(c.then)) {
          for (let j = 0; j < c.then.length; j++) {
            const ch = c.then[j];
            if (ch && (ch.opcode === 'control_exitLoop' || ch.opcode === 'procedures_return') && j < c.then.length - 1) {
              throw new Error("control_exitLoop/return must be the last statement in a substack (found additional statements after break/return)");
            }
      ...
      if (obj.else) {
        ...
        if (Array.isArray(obj.else)) {
          for (let j = 0; j < obj.else.length; j++) {
            const ch = obj.else[j];
            if (ch && (ch.opcode === 'control_exitLoop' || ch.opcode === 'procedures_return') && j < obj.else.length - 1) {
              throw new Error("control_exitLoop/return must be the last statement in a substack (found additional statements after break/return)");
            }
            const childEntry = createBlock(ch, id);
            blocksOut[childEntry.id].parent = id;
    ...
    if (obj.opcode === 'control_while') {
      ...
      if (Array.isArray(bodyArr)) {
        for (let j = 0; j < bodyArr.length; j++) {
          const ch = bodyArr[j];
          if (ch && (ch.opcode === 'control_exitLoop' || ch.opcode === 'procedures_return') && j < bodyArr.length - 1) {
            throw new Error("control_exitLoop/return must be the last statement in a substack (found additional statements after break/return)");
          }
          const childEntry = createBlock(ch, id);
          blocksOut[childEntry.id].parent = id;
      ...
      for (let i = 0; i < parts.length; i++) {
        ...
        if (p == null) {
          ...
          if (omitPh) {
            ...
            const shSpec = { opcode: 'text', inputs: [], fields: { TEXT: ["", makeFieldId()] }, shadow: true };
            const sh = createBlock(shSpec, id);
            blocksOut[sh.id].parent = id;
            blocksOut[sh.id].shadow = true;
            entry.inputs[name] = [3, child.id, sh.id];
          }
        } else if (p && p.opcode) {
    ...
    if (obj.opcode === "procedures_return" && Array.isArray(obj.inputs) && obj.inputs.length > 0) {
      ...
      if (inner && typeof inner === "object" && inner.opcode) {
        ...
        if (omitPh) {
          ...
          const shSpec = { opcode: 'text', inputs: [], fields: { TEXT: ["1", makeFieldId()] }, shadow: true };
          const sh = createBlock(shSpec, id);
          blocksOut[sh.id].parent = id;
          blocksOut[sh.id].shadow = true;
          entry.inputs.return = [3, child.id, sh.id];
        }
      } else {
      ...
      if (p.type === "substack") {
        ...
        if (Array.isArray(val)) {
          for (let j = 0; j < val.length; j++) {
            const ch = val[j];
            if (ch && (ch.opcode === 'control_exitLoop' || ch.opcode === 'procedures_return') && j < val.length - 1) {
              throw new Error("control_exitLoop/return must be the last statement in a substack (found additional statements after break/return)");
            }
            const childEntry = createBlock(ch, id);
          ...
          if (!p.field) {
            ...
            (obj.opcode === 'jwLambda_execute' || obj.opcode === 'jwLambda_executeR') && String(p.name).toUpperCase() === 'LAMBDA') {
              ...
              if (omitPh) {
                ...
                let shSpec = makeShadowForParam(p);
                if (!shSpec) {
                  if (typeCode === InputTypes.math_number) shSpec = { opcode: 'math_number', inputs: [], fields: { NUM: ["", makeFieldId()] }, shadow: true };else shSpec = { opcode: 'text', inputs: [], fields: { TEXT: ["", makeFieldId()] }, shadow: true };
                }
                const sh = createBlock(shSpec, id);
                blocksOut[sh.id].parent = id;
                blocksOut[sh.id].shadow = true;
                entry.inputs[p.name] = [3, child.id, sh.id];
              }
            }
          }
    ...
    if (Array.isArray(obj.children) && obj.children.length > 0) {
      ...
      for (let j = 0; j < obj.children.length; j++) {
        ...
        if (ch && (ch.opcode === 'control_exitLoop' || ch.opcode === 'procedures_return') && j < obj.children.length - 1) {
          throw new Error("control_exitLoop/return must be the last statement in a substack (found additional statements after break/return)");
        }
        const childEntry = createBlock(ch, id);
        blocksOut[childEntry.id].parent = id;
