...
function generateFromAst(astStmts) {
  ...
  function emitSequence(stmts) {
    ...
    for (const st of stmts) {
      ...
      if (!emitted) continue;
      if (Array.isArray(emitted)) seq.push(...emitted);
      else seq.push(emitted);
    }
    return seq;
  }
  ...
  function emitFor(node) {
    ...
    if (node.cond) {
      ...
      if (
        ...
        node.cond.left &&
        node.cond.left.type === "Var" &&
        node.cond.left.name === varName
      ) {
    ...
    if (node.update) {
      if (
        node.update.type === "Assign" &&
        node.update.name === varName &&
        node.update.value &&
        node.update.value.type === "Binary"
      ) {
        // i = i + N  or i = i - N
  ...
  function emitSetVar(name, valueExpr, vtype = "global", srcNode) {
    ...
    let value;
    if (!valueExpr) value = "";
    else if (valueExpr.type === "lit") value = valueExpr.value;
    else if (valueExpr.type === "block") value = valueExpr.block;
    else value = null;
    const out = {
  ...
  function emitCallStmt(stmt) {
    ...
    if (stmt.args && stmt.args.length > 0) {
      for (const arg of stmt.args) {
        ...
        if (res.type === "lit") inputs.push(res.value);
        else inputs.push(res.block);
      }
    }
  ...
  function emitDeclare(stmt) {
    ...
    if (stmt.name.startsWith("__"))
      throw new Error('Variable names starting with "__" are reserved for compiler use: ' + stmt.name);
  ...
  function emitPrint(node) {
    ...
    const inputs = [msg];
    if (hasSecs) {
      if (typeof node.options.seconds === "number") inputs.push(node.options.seconds);
      else {
        const secsRes = emitExpression(node.options.seconds);
        inputs.push(
          secsRes && secsRes.type === "lit"
            ? secsRes.value
            : secsRes && secsRes.type === "block"
              ? secsRes.block
              : 0,
        );
      }
  ...
  function emitExpression(exprNode) {
    ...
    if (Array.isArray(exprNode) || (exprNode && exprNode.type === "RawArray")) {
      ...
      if (blocksMeta && blocksMeta["jwArray_builder"]) {
        ...
        if (exprNode && exprNode.noPlaceholder !== undefined)
          builder.noPlaceholder = !!exprNode.noPlaceholder;
        return {
          type: "block",
          block: builder,
        };
      ...
      for (const el of items) {
        ...
        if (
          ...
          const stmt = emitStmt(el);
          if (stmt) {
            if (Array.isArray(stmt)) arr.push(...stmt);
            else arr.push(stmt);
          }
          continue;
        }
        ...
        if (!rel) continue;
        if (rel.type === "lit") arr.push(rel.value);
        else arr.push(rel.block);
      }
      return {
        type: "block",
        block: arr,
    ...
    if (exprNode.type === "Var") {
      ...
      const vtype =
        exprNode.scope ||
        (varInfo[exprNode.name] && varInfo[exprNode.name].varType
          ? varInfo[exprNode.name].varType
          : "global");
      return {
        type: "block",
        block: {
    ...
    if (exprNode.type === "Ternary") {
      ...
      const thenVal = !exprNode.thenExpr
        ? ""
        : exprNode.thenExpr.type === "Literal"
          ? exprNode.thenExpr.litType === "number"
            ? Number(exprNode.thenExpr.value)
            : String(exprNode.thenExpr.value)
          : emitExpression(exprNode.thenExpr).type === "lit"
            ? emitExpression(exprNode.thenExpr).value
            : emitExpression(exprNode.thenExpr).block;
      const elseVal = !exprNode.elseExpr
        ? ""
        : exprNode.elseExpr.type === "Literal"
          ? exprNode.elseExpr.litType === "number"
            ? Number(exprNode.elseExpr.value)
            : String(exprNode.elseExpr.value)
          : emitExpression(exprNode.elseExpr).type === "lit"
            ? emitExpression(exprNode.elseExpr).value
            : emitExpression(exprNode.elseExpr).block;
      const blk = {
    ...
    if (exprNode.type === "Call") {
      if (exprNode.name === "ask") {
        ...
        const qInput =
          arg && arg.type === "Literal"
            ? arg.litType === "number"
              ...
              : String(arg.value)
            : arg
              ? emitExpression(arg).type === "lit"
                ? emitExpression(arg).value
                : emitExpression(arg).block
              : "";
        const askBlock = {
        ...
        if (exprNode && exprNode.noPlaceholder !== undefined)
          inlineBlock.noPlaceholder = !!exprNode.noPlaceholder;
        return {
          type: "block",
          block: inlineBlock,
          node: exprNode,
      ...
      if (exprNode.name === "jwLambda_newLambda" || exprNode.name === "jwLambda_newLambdaR") {
        ...
        const paramNames = [];
        if (firstArg) {
          if (firstArg.type === "Literal") paramNames.push(String(firstArg.value));
          else if (firstArg.type === "RawArray" && Array.isArray(firstArg.value)) {
      ...
      if (exprNode.args && exprNode.args.length > 0) {
        for (let ai = 0; ai < exprNode.args.length; ai++) {
          ...
          if (a.type === "Literal") {
            ...
            if (!r) inputs.push("");
            else if (r.type === "lit") inputs.push(r.value);
            else inputs.push(r.block);
          }
        }
      }
      const blk = {
    ...
    if (exprNode.type === "Unary") {
      ...
      if (op === "+") {
        if (operand.type === "lit" && typeof operand.value === "number")
          return {
            type: "lit",
...
function nestedToAst(nestedArray) {
  ...
  function convArg(a) {
    ...
    function isStatementLike(node) {
      ...
      if (
        (opcode === "control_while" || type === "While") &&
        ((Array.isArray(node.inputs) && Array.isArray(node.inputs[1])) || node.body)
      )
        return true;
      if (
        (opcode === "SPtempVars_forVar" || opcode === "forVar" || type === "For") &&
        Array.isArray(node.inputs)
      )
        return true;
      if (type === "If" || type === "While" || type === "For" || type === "Break" || type === "Continue")
        return true;
      return false;
    }
  ...
  function convStmt(node) {
    ...
    if (op === "control_while") {
      ...
      const substack =
        inputsArr[1] !== undefined ? inputsArr[1] : Array.isArray(node.children) ? node.children : [];
    ...
    if (op === "SPtempVars_forVar" || op === "forVar") {
      ...
      const substackRaw =
        argsArr[4] !== undefined ? argsArr[4] : Array.isArray(node.children) ? node.children : [];
      const incRaw = argsArr[5] !== undefined ? argsArr[5] : 1;
      const name =
        typeof nameRaw === "string" ? String(nameRaw) : nameRaw && nameRaw.name ? nameRaw.name : "i";
    ...
    if (op === "looks_sayforsecs" || op === "looks_say" || op === "say") {
      const msg =
        Array.isArray(node.inputs) && node.inputs[0] !== undefined
          ? convArg(node.inputs[0])
          : {
              type: "Literal",
              litType: "string",
              value: "",
      ...
      if (op === "looks_sayforsecs") {
        if (Array.isArray(node.inputs) && node.inputs[1] !== undefined)
          opts.seconds = convArg(node.inputs[1]);
      }
      return copyFlagsToAst(
        {
          type: "Print",
    ...
    if (op === "if" || op === "control_expandableIf") {
      ...
      if (Array.isArray(node.cases)) {
        const cases = node.cases.map((c) => {
          ...
          const thenBody = Array.isArray(c.then)
            ? c.then.map(convStmt).filter(Boolean)
            : c.thenBlock && Array.isArray(c.thenBlock.body)
              ? c.thenBlock.body.map(convStmt).filter(Boolean)
              : [];
          return {
            cond: condNode,
            thenBlock: {
        ...
        let elseBlock = null;
        if (node.else) {
          const elseBody = Array.isArray(node.else)
            ? node.else.map(convStmt).filter(Boolean)
            : node.elseBlock && Array.isArray(node.elseBlock.body)
              ? node.elseBlock.body.map(convStmt).filter(Boolean)
              : [];
          elseBlock = {
            type: "Block",
            body: elseBody,
  ...
  for (const item of nestedArray) {
    ...
    if (
      op &&
      (op.startsWith("event_") || op === "click" || op === "flag" || op === "event_whenthisspriteclicked")
    ) {
      let event;
      switch (op) {
      ...
      const children = Array.isArray(item.children)
        ? item.children
        : Array.isArray(item.inputs) && Array.isArray(item.inputs[0])
          ? item.inputs[0]
          : [];
      const body = {
        type: "Block",
