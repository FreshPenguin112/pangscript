...
function generateFromAst(astStmts) {
  ...
  function ensureVariable(name, kind = "let", initExpr) {
    ...
    if (name.startsWith("__")) varInfo[name].varType = "thread";
    return varInfo[name];
  }
  ...
  // Mark the ARRAY input of jwArray_* blocks / OBJECT input of dogeiscutObject_*
  // blocks with the real `noPlaceholder` flag so the converter never emits a
  // placeholder shadow for them. The flag lives on the child filling that input.
  function markArrayObjectNoPlaceholder(block) {
    if (!block || typeof block !== "object" || !block.opcode) return block;
    const op = block.opcode;
    let targetParam = null;
    if (op.startsWith("jwArray_")) targetParam = "ARRAY";else if (op.startsWith("dogeiscutObject_")) targetParam = "OBJECT";
    if (!targetParam) return block;
    const meta = blocksMeta[op];
    if (!meta || !Array.isArray(meta[0])) return block;
    const idx = meta[0].findIndex(p => String(p.name).toUpperCase() === targetParam);
    if (idx < 0) return block;
    const child = block.inputs && block.inputs[idx];
    if (child && typeof child === "object" && child.opcode) child.noPlaceholder = true;
    return block;
  }
  ...
  function emitCallStmt(stmt) {
    ...
    // Special-case `jw_throw(...)` calls emitted by the source preprocessor
    // to become a terminal throw pseudocode block. The block takes exactly
    // one input: the error message (reporter/literal).
    if (stmt.name === "jw_throw") {
      const msg = inputs && inputs.length > 0 ? inputs[0] : "";
      return attachFlagsToPseudo({
        opcode: "control_throw_error",
        inputs: [msg]
      }, stmt);
    }
    //console.log(stmt)
  ...
  function emitDeclare(stmt) {
    ...
    if (stmt.name.startsWith("__")) throw new Error('Variable names starting with "__" are reserved for compiler use: ' + stmt.name);
  ...
  function emitExpression(exprNode) {
    ...
    if (Array.isArray(exprNode) || exprNode && exprNode.type === "RawArray") {
      ...
      if (blocksMeta && blocksMeta["jwArray_builder"]) {
        ...
        for (const el of items) {
          if (!el) continue;
          // If el is a statement-like AST node, emit it as a statement instead of expression
          let rel;
          if (el && typeof el === "object" && el.type && ["If", "While", "For", "Break", "Continue", "Return", "Print", "Assign", "Declare", "Call"].includes(el.type)) {
            const stmt = emitStmt(el);
            if (!stmt) continue;
      ...
      for (const el of items) {
        ...
        if (el && typeof el === "object" && el.type && ["If", "While", "For", "Break", "Continue", "Return", "Print", "Assign", "Declare", "Call"].includes(el.type)) {
          const stmt = emitStmt(el);
          if (stmt) {
            if (Array.isArray(stmt)) arr.push(...stmt);else arr.push(stmt);
          }
          continue;
        }
    ...
    if (exprNode.type === "Var") {
      ensureVariable(exprNode.name);
      const vtype = exprNode.scope || (varInfo[exprNode.name] && varInfo[exprNode.name].varType ? varInfo[exprNode.name].varType : "global");
      return {
        type: "block",
        block: {
          opcode: "SPtempVars_getVar",
          inputs: [vtype, String(exprNode.name)]
        },
        node: exprNode
      };
    }
    ...
    // Object literal emission: prefer dogeiscutObject_builder when available
    if (exprNode.type === "Object") {
      const items = Array.isArray(exprNode.props) ? exprNode.props : [];
      if (blocksMeta && blocksMeta["dogeiscutObject_builder"]) {
        const substack = [];
        for (const p of items) {
          if (!p) continue;
          const keyNode = p.key !== undefined && p.key !== null ? p.key : "";
          const keyExpr = p.computed && p.computedExpr ? p.computedExpr : keyNode && typeof keyNode === "object" ? keyNode : null;
          let keyVal;
          if (typeof keyNode === "string") {
            keyVal = String(keyNode);
          } else if (keyExpr) {
            const emittedKey = emitExpression(keyExpr);
            if (emittedKey && emittedKey.type === "lit") keyVal = emittedKey.value;else if (emittedKey && emittedKey.type === "block") keyVal = emittedKey.block;else if (emittedKey) keyVal = emittedKey;else keyVal = keyExpr;
          } else if (keyNode && typeof keyNode === "object") {
            const emittedKey = emitExpression(keyNode);
            if (emittedKey && emittedKey.type === "lit") keyVal = emittedKey.value;else if (emittedKey && emittedKey.type === "block") keyVal = emittedKey.block;else if (emittedKey) keyVal = emittedKey;else keyVal = keyNode;
          } else {
            keyVal = keyNode;
          }
          if (!p.value) {
          ...
          const valRes = emitExpression(p.value);
          if (!valRes) continue;
          const val = valRes.type === "lit" ? valRes.value : valRes.block;
          const appendBlk = {
        ...
        const builder = {
          ...
          inputs: [{
            opcode: "dogeiscutObject_currentObject",
            shadow: true,
            noPlaceholder: true
          }, substack]
        };
      ...
      // Fallback: emit a plain object mapping as a literal block (preserve old behavior)
      const obj = {};
      for (const p of items) {
        if (!p) continue;
        const keyNode = p.key !== undefined ? p.key : "";
        const keyExpr = p.computed && p.computedExpr ? p.computedExpr : keyNode && typeof keyNode === "object" ? keyNode : null;
        const key = typeof keyNode === "string" ? keyNode : keyExpr ? (() => {
          const emittedKey = emitExpression(keyExpr);
          if (emittedKey && emittedKey.type === "lit") return emittedKey.value;
          if (emittedKey && emittedKey.type === "block") return emittedKey.block;
          return emittedKey;
        })() : keyNode;
        const valRes = p.value ? emitExpression(p.value) : null;
        obj[key] = valRes ? valRes.type === "lit" ? valRes.value : valRes.block : null;
      }
      return {
        type: "block",
        block: obj
    ...
    if (exprNode.type === "Call") {
      ...
      // jwArray / dogeiscutObject blocks must never emit a placeholder shadow on
      // their ARRAY / OBJECT input. Mark the child at that input position with
      // the real `noPlaceholder` flag right where the block is materialized.
      markArrayObjectNoPlaceholder(blk);
      if (exprNode && exprNode.shadow !== undefined) blk.shadow = !!exprNode.shadow;
      if (exprNode && exprNode.noPlaceholder !== undefined) blk.noPlaceholder = !!exprNode.noPlaceholder;
      return {
        type: "block",
...
function nestedToAst(nestedArray) {
  function copyFlagsToAst(out, src) {
    ...
    if (src.generator !== undefined) out.generator = !!src.generator;
    return out;
  }
  function convArg(a) {
    // Helper: check if a node is statement-like (should be converted via convStmt, not convArg)
    function isStatementLike(node) {
      if (!node || typeof node !== "object") return false;
      const opcode = node.opcode || node.name;
      const type = node.type;
      // If statements with cases, while/for loops, breaks, etc. should be statements, not expressions
      if ((opcode === "if" || opcode === "control_expandableIf") && Array.isArray(node.cases)) return true;
      if ((opcode === "control_while" || type === "While") && (Array.isArray(node.inputs) && Array.isArray(node.inputs[1]) || node.body)) return true;
      if ((opcode === "SPtempVars_forVar" || opcode === "forVar" || type === "For") && Array.isArray(node.inputs)) return true;
      if (type === "If" || type === "While" || type === "For" || type === "Break" || type === "Continue") return true;
      return false;
    }
    ...
    // For statement-like nodes, use convStmt instead of convArg.
    if (Array.isArray(a)) {
      return {
        type: "RawArray",
        value: a.map(el => typeof el === "object" && el.opcode ? convStmt(el) : convArg(el))
      };
    }
    if (a == null) return {
      type: "Literal",
    ...
    if (typeof a === "object") {
      ...
      // If statement with cases/else (from nested stmtToNested) appearing in a substack array.
      // Convert it to an If AST node instead of a Call.
      if ((opcode === "if" || opcode === "control_expandableIf") && Array.isArray(a.cases)) {
        const cases = a.cases.map(c => {
          const condNode = c && c.cond !== undefined ? convArg(c.cond) : {
            type: "Literal",
            litType: "boolean",
            value: false
          };
          const thenBody = Array.isArray(c.then) ? c.then.map(convStmt).filter(Boolean) : [];
          return {
            cond: condNode,
            thenBlock: {
              type: "Block",
        ...
        let elseBlock = null;
        if (a.else !== undefined && a.else !== null) {
          const elseBody = Array.isArray(a.else) ? a.else.map(convStmt).filter(Boolean) : [];
          elseBlock = {
            type: "Block",
            body: elseBody
          };
