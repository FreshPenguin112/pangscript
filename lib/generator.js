...
function generateFromAst(astStmts) {
  ...
  function emitFor(node) {
    ...
    return {
      ...
      inputs: ['thread', varName, startVal, endVal, body, incVal]
    };
  }
  function emitSetVar(name, valueExpr, vtype = "global") {
    ensureVariable(name);
  ...
  function emitAssign(stmt) {
    ...
    if (stmt.varType) return emitSetVar(stmt.name, value, stmt.varType);
  ...
  for (const s of astStmts) {
    if (s.type === "On") {
      ...
      outBlocks.push({ opcode, children });
    } else {
...
function nestedToAst(nestedArray) {
  ...
  function convStmt(node) {
    ...
    if (op === 'control_while') {
      const inputsArr = Array.isArray(node.inputs) ? node.inputs : Array.isArray(node.args) ? node.args : [];
    ...
    if (op === 'SPtempVars_forVar' || op === 'forVar') {
      const argsArr = Array.isArray(node.inputs) ? node.inputs : Array.isArray(node.args) ? node.args : [];
